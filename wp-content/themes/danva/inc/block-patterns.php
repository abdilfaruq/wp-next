 ...
 * @return void
 */
function danva_register_block_patterns()
{
    $block_pattern_categories = array(
        'danva' => array('label' => esc_html__('danva', 'danva')),
        'pages'  => array('label' => esc_html__('Pages', 'danva')),
    );
     ...
     * }
     */
    $block_pattern_categories = apply_filters('danva_block_pattern_categories', $block_pattern_categories);
        ...
        if (is_file($file_path) && pathinfo($file_path, PATHINFO_EXTENSION) == 'php') {
            ...
            if ($file !== 'pricing-table-2.php') {
                ...
                if (!empty($pattern_content)) {
                    ...
                    $pattern_name = 'danva-pattern-' . sanitize_title($pattern_title); // Generate a unique name for the pattern
...
add_action('init', 'danva_register_block_patterns', 9);
